...
async def _(event):
    ...
    packname = f"@apaloeliat pack {userid}"
    packshortname = f"Montamantoel_{userid}"  # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    pack += 1
                    prevv = int(pack) - 1
                    packname = f"{user.first_name}'s Poco bot Vol.{pack}"
                    ...
                    if not await stickerset_exists(bot_conv, packshortname):
                        ...
                        if not response.text.startswith("Alright!"):
                            if "unacceptable" in response.text:
                                packname = f"{user.id}'s Poco Bot Vol.{pack}"
                                response = await silently_send_message(bot_conv, packname)
                            else:
                                await event.edit(f"**FAILED**! @Stickers replied: {response.text}")
                        w = await bot_conv.send_file(
